...
uint8_t rData[30];                  // Send data. 
// uint8_t r;                  // Send data. 

int len = sizeof(sData);
char rxData[] = "e from ESP..I got the messag\0";
// char rData[];
char *tx;
char *rx;
...
// __interrupt void INT_mySPIM_RX_ISR(void){
    // msg = "\r\nM_RX\0";
    // SCI_writeCharArray(mySCIA_BASE, (uint16_t*)msg, 7);


    // SPI_resetRxFIFO(mySPIM_BASE);
    // Interrupt_clearACKGroup(INT_mySPIM_RX_INTERRUPT_ACK_GROUP);
// }


// __interrupt void INT_mySPIM_TX_ISR(void){
//     msg = "\r\nM_TX\0";
//     SCI_writeCharArray(mySCIA_BASE, (uint16_t*)msg, 7);


//     SPI_resetTxFIFO(mySPIM_BASE);
//     Interrupt_clearACKGroup(INT_mySPIM_TX_INTERRUPT_ACK_GROUP);
// }
